...
Callback_PlayerConnect()
{
	...
	//---fix variable reset in round based gamemodes----//
	
	if ( !isDefined( self.pers["numberofstreaks"] ) )
			self.pers["numberofstreaks"] = 0;	
			
	if ( !isDefined( self.pers["doSuperDamage"] ) )
			self.pers["doSuperDamage"] = 0;
	
	if ( !isDefined( self.pers["streaknumber"][self.pers["numberofstreaks"]] ) )
			self.pers["streaknumber"][self.pers["numberofstreaks"]] = 0;
			
	if ( !isDefined( self.pers["durationnumber"][self.pers["numberofstreaks"]] ) )
			self.pers["durationnumber"][self.pers["numberofstreaks"]] = 0;		
...
kickIfDontSpawn()
{
	...
	waittime = getdvarfloat("scr_kick_time", 90000 );
	mintime = getdvarfloat("scr_kick_mintime", 450000 );
